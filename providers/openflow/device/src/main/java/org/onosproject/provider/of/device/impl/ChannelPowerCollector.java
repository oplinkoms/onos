 ...
 */
public class ChannelPowerCollector implements TimerTask {
     ...
     */
    public ChannelPowerCollector(OpenFlowSwitch sw, int interval) {
        this.sw = sw;
    ...
    private void sendChannelPowerStatistic() {
        if (sw.getRole() != RoleState.MASTER) {
            return;
        }
        Long statsXid = xidAtomic.getAndIncrement();
        OFOplinkChannelPowerRequest statsRequest = sw.factory().buildOplinkChannelPowerRequest()
                .setXid(statsXid)
                .build();
    ...
    public synchronized void start() {
        log.info("Starting Channel Power Stats collection thread for {}", sw.getStringId());
        stopped = false;
    ...
    public synchronized void stop() {
        log.info("Stopping Channel Power Stats collection thread for {}", sw.getStringId());
        stopped = true;
        timeout.cancel();
    }
}
