...
public class OpenFlowDeviceProvider extends AbstractProvider implements DeviceProvider {
    ...
    public void changePortState(DeviceId deviceId, PortNumber portNumber,
        ...
        pmb.setPortNo(port);
        Set<OFPortConfig> portConfig = new HashSet<>();
        if (enable) {
        ...
        Set<OFPortConfig> portMask = new HashSet<>();
        portMask.add(OFPortConfig.NO_STP);
                ...
                case OPTICAL_AMPLIFIER:
                    ...
                    opsw.getPortTypes().forEach(type -> {
                        opsw.getPortsOf(type).forEach(
                                op -> {
                                    portDescs.add(buildPortDescription(type, (OFPortOptical) op, opsw));
                                }
                        );
                    });
                    break;
